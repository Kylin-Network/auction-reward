...
async function main() {
    ...
    buf.writeUInt32LE(fund_info["fundIndex"]);
    ...
    let network_prefix = parseInt(await api.consts.system.ss58Prefix.toString());
    let para_network_prefix = 42;
    if (args["set-ss58-format"]) {
        para_network_prefix = args["set-ss58-format"];
    }
    ...
    for (let i = 0; i < storages.length; i++) {
        ...
        if (storage.isSome) {
            ...
            json.contributions.push({
                ...
                "para_account": encodeAddress(all_keys[i].toHex(), para_network_prefix),
    ...
    console.log('total contributions:', json.contributions.length)
